import Config

# Configure your database
#
# The MIX_TEST_PARTITION environment variable can be used
# to provide built-in test partitioning in CI environment.
# Run `mix help test` for more information.

# In test we don't send emails.
config :uwu_blog, UwUBlog.Mailer, adapter: Swoosh.Adapters.Test

# Login page path for the auth tests.
config :uwu_blog, UwUBlogWeb.Auth, login_path: "/login"

# Google OAuth test config. Req calls are routed to a per-test stub
# (`Req.Test`), so no real network requests are made.
config :uwu_blog, UwUBlogWeb.Auth.Google,
  client_id: "test-client-id",
  client_secret: "test-secret",
  allowed_emails: "admin@example.com",
  req_options: [plug: {Req.Test, UwUBlogWeb.Auth.Google}]

config :uwu_blog, UwUBlog.Repo,
  username: "postgres",
  password: "postgres",
  hostname: "localhost",
  database: "uwu_blog_test#{System.get_env("MIX_TEST_PARTITION")}",
  pool: Ecto.Adapters.SQL.Sandbox,
  pool_size: 10

# Enable the SQL sandbox plug/on_mount so concurrent LiveView + controller
# tests can share a transactional connection.
config :uwu_blog, sql_sandbox: true

# The now-playing worker polls the DB at boot; don't start it under the test
# sandbox (nothing in the suite exercises it).
config :uwu_blog, start_now_playing: false

# We don't run a server during test. If one is required,
# you can enable the server option below.
config :uwu_blog, UwUBlogWeb.Endpoint,
  http: [ip: {127, 0, 0, 1}, port: 4002],
  secret_key_base: "xF2/vbb/5R/pAj1NZBuPzPXUm5hOg4a+3VVz5AhuNpobdi4q04bfHn7rcmX/mJDR",
  server: false

config :logger, level: :warning
config :phoenix, :plug_init_mode, :runtime

config :phoenix_live_view,
  # Disable swoosh api client as it is only required for production adapters.
  enable_expensive_runtime_checks: true

config :swoosh, :api_client, false
